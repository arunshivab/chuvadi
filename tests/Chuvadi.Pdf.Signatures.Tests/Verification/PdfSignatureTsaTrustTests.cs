...
// PHASE: Phase 1.1.4 — TSA chain validation tests
//
// Fixture: signing chain (SignRoot → SignIntermediate → Leaf), separate TSA
// chain (TsaRoot → TsaLeaf with id-kp-timeStamping EKU). PDF carries the
// signature plus an embedded id-aa-signatureTimeStampToken whose CMS
// signature chains to the TSA root.
...
public sealed class PdfSignatureTsaTrustTests
{
    ...
    private const string TsaRootCertDerHex =
        "308202d9308201c1a003020102020164300d06092a864886f70d01010b0500301c311a3018060355" +
        "04030c1154534120546573742054534120526f6f74301e170d3230303130313030303030305a170d" +
        "3430303130313030303030305a301c311a301806035504030c115453412054657374205453412052" +
        "6f6f7430820122300d06092a864886f70d01010105000382010f003082010a0282010100bc4895fb" +
        "acfb59a8b7a2f7ac5744d7e75591e9cc67e75277d5038efe45f0f0352235bd0f01a47a21de2712e6" +
        "11a1444e73956f4f27e04356084d5b92c581b09a24b2fbc63eb4af7bdbbd1ecfeef4040edd3abb31" +
        "e64cb0befdaf9a8f559ed3752b10901e122bc8d340a67e969b885affe40c28d8afccdf518383ead4" +
        "90ea6ed3b8d0217232da8e0ee5a303ea0eda76f127603ae61db09cfd8318d11c3d66a68c6c7a0a41" +
        "12669662893999a409b95c3c624e034147b3dd98c0d006181b2c51b5ac1d14a213564c21d1a90561" +
        "7fe068f16702761c8a7a3447fc026ee40abfd29ef857edf669ca43a9bdb4717bbfc54d32bc7ba558" +
        "f9ccc3dede4f1639f38281990203010001a326302430120603551d130101ff040830060101ff0201" +
        "01300e0603551d0f0101ff040403020106300d06092a864886f70d01010b05000382010100295e63" +
        "ee5a4561f1916112702564bad6b8209d8a471f17924de1bb983c88ad4b5dbbaa47f86c9eac5f7fac" +
        "43fabe84caff271250c17e2e1c94079c02e189300f6426f4a862f3da72506243d25358c19ce78393" +
        "726210a7c176da170e01615353b08f28b28906a6449770170a3104b7095b92cccd4a0c6b535f1cfa" +
        "e71c9f6904db76d90d396e684bf83d6750348c489937acdd7688b0826f709f3e7eadf4232bbd8f05" +
        "74a4948f766e178cfd8d1eabf694aaefd221cbc6fbc4cd35a0b022529c011db7bcf4b7c45daee9ac" +
        "3ed79ede35cc47530877a2edc32e1682225ab508030558e726487d03b24080f6a33653f2dd310a46" +
        "3d63af3c2adab43061ed8df656";
    ...
    private static TrustStore TsaTrustStoreCorrect()
    {
        TrustStore s = new();
        s.Add(X509Certificate.Decode(Convert.FromHexString(TsaRootCertDerHex)));
        return s;
    }

    [Fact]
    public void Verify_NoTsaTrustStore_TimestampTrustNotEvaluated()
    {
        ...
        r.TimestampTrustValidated.Should().BeFalse();
        r.TimestampValidatedPath.Should().BeNull();
    }

    [Fact]
    public void Verify_CorrectTsaTrustStore_TimestampTrustValidated()
    {
        using PdfDocument doc = BuildPdf();
        SignatureVerifyOptions opts = new()
        {
            TrustStore = SigningTrustStore(),
            TsaTrustStore = TsaTrustStoreCorrect(),
            ValidationTime = GoodTime,
        };
        SignatureVerificationResult r = doc.Signatures()[0].Verify(doc, opts);
        r.IsValid.Should().BeTrue();
        r.TimestampValidated.Should().BeTrue();
        r.TimestampTrustValidated.Should().BeTrue();
        r.TimestampValidatedPath.Should().NotBeNull();
    }

    [Fact]
    public void Verify_WrongTsaTrustStore_TimestampTrustNotValidated()
    {
        using PdfDocument doc = BuildPdf();
        // TSA trust store contains the SIGNING root — wrong issuer for the TSA chain.
        TrustStore wrong = new();
        wrong.Add(X509Certificate.Decode(Convert.FromHexString(SignRootCertDerHex)));
        SignatureVerifyOptions opts = new()
        {
            TrustStore = SigningTrustStore(),
            TsaTrustStore = wrong,
