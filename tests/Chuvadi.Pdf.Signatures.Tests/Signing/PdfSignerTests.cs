...
// PHASE: Phase 1.2.1 — High-level PDF signing API
//
// End-to-end: build an unsigned PDF, sign it with PdfSigner, re-open, run
// signature.Verify against the embedded self-signed cert as the trust
// anchor. Plus tamper-detection coverage.
...
public sealed class PdfSignerTests
{
    ...
    private static (ISigner Signer, X509Certificate Cert) BuildSigner()
    {
        ...
        return (new RsaPkcs1V15Signer(priv, cert, HashAlgorithmName.Sha256), cert);
    }

    [Fact]
    public void Sign_ProducesAVerifiableSignature()
    {
        (ISigner signer, X509Certificate cert) = BuildSigner();
        ...
        using PdfDocument signedDoc = PdfDocument.Open(new MemoryStream(signedBytes), leaveOpen: false);
        signedDoc.Signatures().Should().HaveCount(1);

        SignatureVerificationResult r = signedDoc.Signatures()[0].Verify(signedDoc, new()
        {
            TrustStore = trust,
    ...
    [Fact]
    public void Sign_EmbedsReasonAndLocationInTheSignatureDictionary()
    {
        (ISigner signer, _) = BuildSigner();
        byte[] signedBytes = SignFreshPdf(signer, new Chuvadi.Pdf.Signatures.Signing.PdfSigningOptions
        {
            SigningTime = SigningTime,
            Reason = "I approve this document",
            Location = "Pune, India",
            ContactInfo = "arun@example.com",
        });

        using PdfDocument signedDoc = PdfDocument.Open(new MemoryStream(signedBytes), leaveOpen: false);
        PdfSignature sig = signedDoc.Signatures()[0];
        sig.Reason.Should().Be("I approve this document");
        sig.Location.Should().Be("Pune, India");
        sig.SigningTimeFromDictionary.Should().NotBeNull();
    }

    [Fact]
    public void Sign_TamperingABytePastTheSignatureBreaksIntegrity()
    {
        (ISigner signer, X509Certificate cert) = BuildSigner();
        TrustStore trust = new();
        trust.Add(cert);
    ...
    [Fact]
    public void Sign_CustomPlaceholderSize_StillProducesVerifiableSignature()
    {
        (ISigner signer, X509Certificate cert) = BuildSigner();
        TrustStore trust = new();
        trust.Add(cert);
    ...
    [Fact]
    public void Sign_PlaceholderTooSmall_Throws()
    {
        (ISigner signer, _) = BuildSigner();
        // 256 bytes is far less than what an RSA-2048 CMS needs (~1.2 KB)
        Action act = () => SignFreshPdf(signer,
            new Chuvadi.Pdf.Signatures.Signing.PdfSigningOptions
            {
                SigningTime = SigningTime,
                ContentsPlaceholderSize = 256,
            });
        act.Should().Throw<InvalidOperationException>()
            .WithMessage("*exceeds*placeholder*");
    }

    [Fact]
    public void Sign_NullDocument_Throws()
    {
        (ISigner signer, _) = BuildSigner();
        Action act = () => Chuvadi.Pdf.Signatures.Signing.PdfSigner.Sign(
            null!, signer, new Chuvadi.Pdf.Signatures.Signing.PdfSigningOptions());
        act.Should().Throw<ArgumentNullException>();
    }

    [Fact]
    public void Sign_NullSigner_Throws()
    {
        using PdfDocument doc = PdfDocument.Open(new MemoryStream(BuildMinimalPdf()), leaveOpen: false);
        Action act = () => Chuvadi.Pdf.Signatures.Signing.PdfSigner.Sign(
            doc, null!, new Chuvadi.Pdf.Signatures.Signing.PdfSigningOptions());
        act.Should().Throw<ArgumentNullException>();
    }

    [Fact]
    public void Sign_NullOptions_Throws()
    {
        (ISigner signer, _) = BuildSigner();
        using PdfDocument doc = PdfDocument.Open(new MemoryStream(BuildMinimalPdf()), leaveOpen: false);
        Action act = () => Chuvadi.Pdf.Signatures.Signing.PdfSigner.Sign(doc, signer, null!);
        act.Should().Throw<ArgumentNullException>();
    }
