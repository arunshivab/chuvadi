...
// PHASE: Phase 1.2.6 — RSA-PSS signing
...
public sealed class RsaPssSignerTests
{
    ...
    [Fact]
    public void Sign_ReturnsModulusSizedSignature()
    {
        var (signer, priv, _) = MakeSigner();
        byte[] data = "pss test data"u8.ToArray();
        byte[] sig = signer.Sign(data);
        sig.Length.Should().Be(priv.ModulusSizeBytes);
    }

    [Fact]
    public void Sign_VerifiesAgainstRsaVerifierPss()
    {
        var (signer, priv, _) = MakeSigner();
        byte[] data = "verifiable pss"u8.ToArray();
        byte[] sig = signer.Sign(data);
        ...
        RsaPublicKey pub = new(priv.Modulus, priv.PublicExponent);
        RsaVerifier.VerifyPss(pub, HashAlgorithmName.Sha256, HashAlgorithmName.Sha256, 32, digest, sig)
            .Should().BeTrue();
    }

    [Fact]
    public void Sign_DifferentCallsProduceDifferentSignatures()
    {
        // PSS is probabilistic — fresh salt each call.
        var (signer, _, _) = MakeSigner();
        byte[] data = "pss randomness"u8.ToArray();
        byte[] s1 = signer.Sign(data);
        byte[] s2 = signer.Sign(data);
        Convert.ToHexString(s1).Should().NotBe(Convert.ToHexString(s2));
    }

    [Fact]
    public void SignatureAlgorithm_HasPssOid()
    {
        var (signer, _, _) = MakeSigner();
        signer.SignatureAlgorithm.Algorithm.ToString().Should().Be("1.2.840.113549.1.1.10");
    }

    private static (RsaPssSigner Signer, RsaPrivateKey Priv, X509Certificate Cert) MakeSigner()
    {
        ...
        return (new RsaPssSigner(priv, cert, HashAlgorithmName.Sha256), priv, cert);
    }
}
